...
# Hai Perikod:) 
     ...
     # Awas Broo Script Ini Ada Logger Nya
     # Jan dipake Nanti Error
     # Ada Virus an Broo
     # Alert Buat Perikod
...
def mobile(uiz,pwr,link_okep):
	...
	sys.stdout.write("\r %s %s/%s %sok:%s %scp:%s "%(P,len(id2 or id4),HikmatXD,H,ok,K,cp)); sys.stdout.flush()
	#c_pw -=1
	for pw in pwr:
		pw = pw.lower()
		try:
...
def cek_apk(ses,kuki):
	...
	sop = sop(__Hikmat__,"html.parser")
